...
pub struct ForgottenKey<T: ?Sized + Any>(usize, PhantomData<T>);

impl<T: ?Sized + Any> ForgottenKey<T> {
    #[inline]
...
impl<T: ?Sized + Any> Debug for ForgottenKey<T> {
...
impl<T: ?Sized + Any> Drop for ForgottenKey<T> {
    fn drop(&mut self) {
        if self.0 != 0 {
